...
static int cmp_vmrnode_and_va(const void *va, const struct rb_node *node)
{
    struct vmregion *vmr;
    vaddr_t target = *(const vaddr_t *)va;
    vaddr_t vmr_start, vmr_end;
...
struct vmregion *find_vmr_for_va(struct vmspace *vmspace, vaddr_t addr)
{
        ...
        search = rb_search(&(vmspace->vmr_tree), &addr, cmp_vmrnode_and_va);

        if (!search) {
                return NULL;
        }

        return rb_entry(search, struct vmregion, tree_node);
