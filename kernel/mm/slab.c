...
static void choose_new_current_slab(struct slab_pointer *pool)
{
        ...
        struct list_head *partial = &(pool->partial_slab_list);

        if (list_empty(partial))
        {
        ...
        else
        {
                ...
                slab = (struct slab_header *)list_entry(partial->next, struct slab_header, node);
                pool->current_slab = slab;
                list_del(partial->next);
        }        
...
void free_in_slab(void *addr)
{
        ...
        struct slab_slot_list *free_list = slab->free_list_head;
        slot->next_free = free_list;
